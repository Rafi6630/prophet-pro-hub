    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground mt-0.5 truncate">
          {ticket.userName} · {t(CATEGORY_KEY_MAP[ticket.category])} · {timeAgo(ticket.updatedAt)}
        </p>
      </div>
      ...
      <div className="rounded-2xl bg-card border border-border overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {Object.entries(CATEGORY_KEY_MAP).map(([key, labelKey]) => {
                const cats = tickets.filter(tk => tk.category === key);
                if (!cats.length) return null;
                const open = cats.filter(tk => tk.status === "open" || tk.status === "in_progress").length;
                const res  = cats.filter(tk => tk.status === "resolved" || tk.status === "closed").length;
                return (
                  <tr key={key} className="border-b border-border last:border-0 hover:bg-secondary/20">
                    <td className="px-4 py-3 font-medium text-foreground">{t(labelKey)}</td>
