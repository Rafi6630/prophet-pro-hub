    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-card border border-border p-4 space-y-3">
        ...
        <div className="flex gap-1.5 flex-wrap">
          {DEAL_TYPES.map(dt => (
            <button key={dt} onClick={() => setTypeFilter(dt)}
              className={`px-3 py-1 rounded-xl text-xs font-medium border transition-all ${typeFilter === dt ? "bg-primary text-white border-primary" : "bg-secondary/40 border-border text-foreground hover:border-primary/40"}`}>
              {dt}
            </button>
          ))}
        </div>
